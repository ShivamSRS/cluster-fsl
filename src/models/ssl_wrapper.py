...
class SSLWrapper(BaseWrapper):
    ...
    def get_episode_dict(self, batch):
        ...
        episode_dict["my"] = {'labels':labels[ns+nq:]}
        return episode_dict

    def predict_on_batch(self, episode_dict, support_size_max=None):
        """
        dict(
            support=dict(samples=x, labels=y),
            query=dict(samples=x, labels=y),
            unlabeled=dict(samples=x),
            n_classes=int,
        )
        x: shape(n, d)
        y: shape(n,)
        ...
        :param episode_dict:
        :param support_size_max:
        :return:
        """
        ind_selected = sm.get_indices(selection_method=self.exp_dict['selection'],
    ...
    def val_on_batch(self, batch):
        ...
        pred_labels = self.predict_on_batch(episode_dict,
                                            support_size_max=self.exp_dict['unlabeled_size_test']*self.exp_dict['classes_test'])
    ...
    def test_on_loader(self, data_loader, max_iter=None):
        ...
        with tqdm.tqdm(total=len(data_loader), ascii='->', ncols=80) as pbar:
