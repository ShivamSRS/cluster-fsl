...
class FinetuneWrapper(BaseWrapper):
    ...
    def __init__(self, model, nclasses, exp_dict):
        ...
        print('============> {} = {}'.format(key, self.exp_dict[key]))
        ...
        # self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=self.exp_dict["max_epoch"], eta_min=self.exp_dict["min_lr_decay"])
        self.model.cuda()
        if self.ngpu > 1:
    ...
    def train_on_batch(self, batch):
        ...
        unlabeled_size = episode["unlabeled_size"]
        labels = episode["targets"][:(support_size + query_size)*nclasses, ...].view(support_size + query_size, nclasses, -1).cuda(non_blocking=True).long()
